...
class Pilote(toga.App):
        ...
        def __init__(self,url):
                ...
                if(url):
                        ...
                        asyncio.create_task(self.__initialize_client(url))
                else :
                        not_found = toga.Label ("Pypilot not found")  
                        self.__box  = toga.Box(url, flex=1, children=[not_found])
                pass
                
                        
        async def __initialize_client(self,url):
                self.client = False
                self.__connect(url)
                self.__getMessages()
                if self.__last_val(self.ENABLED) == False:
                        self.__update_enable(False)

                while True:
                        
                        if self.client:
                                self.__label_deconnected.visibility="hidden"
                                self.__getMessages()
        ...
        def __initialize_pilote_box(self):
                ...
                self.__label_deconnected = toga.Label ("Deconnected",                                                        style=self.style.small)
                
                child_box1 = toga.Box(style=self.style.child_box, children=[self.__button_m1,      self.__button_p1])
                child_box2 = toga.Box(style=self.style.child_box, children=[self.__button_m10,     self.__button_p10])
                child_box3 = toga.Box(style=self.style.child_box, children=[self.__button_compass, self.__button_nav])
                child_box4 = toga.Box(style=self.style.child_box, children=[self.__button_wind,    self.__button_true_wind])
                child_box5 = toga.Box(style=self.style.child_box, children=[self.__button_gps,     self.__button_disengage])
                
                self.__box = toga.Box("Pilote", style=self.style.main_box, children=[self.__label_target,child_box1,child_box2,child_box3,child_box4,child_box5,self.__label_deconnected ])
                pass
        ...
        def __disengage(self, widget):
                if self.client:
                        ...
                        self.__set(self.MODE, self.MODE_GPS)
        ...
        def __update_mode(self, mode):
                ...
                self.__button_true_wind.background_color = self.style.COLOR_OFF
                self.__button_nav.background_color       = self.style.COLOR_OFF
                if self.__last_val(self.ENABLED) == True:
...
########################### MyClient ###########################
        ...
        def __connect(self,url):
                print("Pypilote Ip : " + url)
                watchlist = [self.ENABLED, self.MODE, self.COMMAND, self.HEADING]
                self.last_msg = {}

                try:
                    self.client = pypilotClientFromArgs(url)

                    for name in watchlist:
                        self.client.watch(name)

                    print('Pypilote connected')
        ...
        def __getMessages(self):
                ...
                try:
                        ...
                        self.__label_deconnected.visibility="visible"
                        self.client = False
                ...
                for name, value in msgs.items():
                        ...
                        match name:
                                ...
                                case self.HEADING:
                                        ...
                                        if self.__last_val(self.ENABLED) == False:
                                                self.__label_target.text=str(round(value))
                return True
